...
files = dir('data/Results/*.jpg');

P_Mat = load( fullfile( 'data/dino', 'dino_Ps.mat') );
P_Mat = P_Mat.P;
...
for k=1:size(files)
    %% Get new mask
    filename = sprintf("data/Results/img%01d.jpg",(k));
    img = imread(filename);
    mask = silhouette(img);
    imshow(img);
    pause();
    ...
    for i=1:res % xAxis
        ...
        for j=1:res % yAxis
            ...
            for l=1:res % zAxis
                ...
                vi = [a b c 1];
                %pr = P_Mat{1,k}(:,:)*vi';
                pr = projection_matrices(k+3)*vi';
    
                x = floor(pr(1)/pr(3));
                y = floor(pr(2)/pr(3));
                
                if y<256 && x<=256 && x>=1 && y>=1
                    if mask(y,x) == 0
                        box(i,j,l) = 0;
                    end
                end                
